...
def merge_trees(root1, root2):
    def mergeTreesHelper(node1, node2):
        ...
        left = mergeTreesHelper(
            node1.left if node1 else None, node2.left if node2 else None
        )
        right = mergeTreesHelper(
            node1.right if node1 else None, node2.right if node2 else None
        )
