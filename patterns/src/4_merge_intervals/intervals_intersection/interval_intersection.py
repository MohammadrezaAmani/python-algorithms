def interval_intersection(interval_a, interval_b):
    ...
    while i < len(interval_a) and j < len(interval_b):
        a_overlaps_b = (
            interval_a[i][start] >= interval_b[j][start]
            and interval_a[i][start] <= interval_b[j][end]
        )

        b_overlaps_a = (
            interval_b[j][start] >= interval_a[i][start]
            and interval_b[j][start] <= interval_a[i][end]
        )
